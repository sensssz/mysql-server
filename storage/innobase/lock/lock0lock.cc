...
public:
    ...
    getMutex(
        ...
        mutex_enter(&mutex);
        auto itr = mutex_queues.find(t);
        if (itr == mutex_queues.end()) {
            mutex_create(LATCH_ID_LOCK_SYS, &mutex_queues[t]);
        }
        wait_queue[t];
        read_queue[t];
...
unordered_map<triplet, LockMutex, hash_triplet> RT::mutex_queues;
LockMutex RT::mutex;
LockMutex RT::qMutex;
static
void
...
DeadlockChecker::get_first_lock(ulint* heap_no) const
{
	...
	ut_a(lock != NULL);
	ut_a(lock != m_wait_lock || use_vats(lock->trx) || use_ldsf(lock->trx));
