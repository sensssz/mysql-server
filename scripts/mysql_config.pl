...
my $flags;
$flags->{libs} = [@lib_opts, qw(-lz)];
$flags->{embedded_libs} = [@lib_e_opts, qw(-lz)];

$flags->{include} = ["-I$pkgincludedir"];
$flags->{cflags}  = [@{$flags->{include}},split(" ",'-D_FORTIFY_SOURCE=2 -fno-omit-frame-pointer')];
$flags->{cxxflags}= [@{$flags->{include}},split(" ",'-D_FORTIFY_SOURCE=2 -fno-omit-frame-pointer')];
